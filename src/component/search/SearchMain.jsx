...
const SearchMain = () => {
  ...
  const handlePlaceSelect = useCallback((place, showDetail = false) => {
    ...
    content.innerHTML = `
      <div style="background:white; border-radius:8px; border:2px solid #2d6cff; padding:10px; width:180px; box-shadow:0 2px 8px rgba(0,0,0,0.15);">
        ${place.imageUrls?.[0] ? `<img src="${place.imageUrls[0]}" style="width:100%; height:80px; object-fit:cover; border-radius:4px;" />` : ''}
  ...
  useEffect(() => {
    ...
    content.innerHTML = `
      <div style="background:white; border-radius:8px; border:2px solid #2d6cff; padding:10px; width:180px; box-shadow:0 2px 8px rgba(0,0,0,0.15);">
        ${selectedPlace.imageUrls?.[0] ? `<img src="${selectedPlace.imageUrls[0]}" style="width:100%; height:80px; object-fit:cover; border-radius:4px;" />` : ''}
        <div style="margin-top:6px; font-size:14px; font-weight:bold; color:#2d6cff;">${selectedPlace.title}</div>
      </div>
    `;
          ...
          {selectedPlace ? (
            ...
            <>
              ...
              <CategorySelector
                ...
                categories={['관광지', '액티비티', '식당', '카페', '쇼핑', '숙소']}
              />
              <hr className="category-divider" />
              <div key={category} className="list-wrapper fade-in">
                <PlaceListView places={places} onSelect={place => handlePlaceSelect(place, true)} />
              </div>
            </>
          )}
        </div>
